...
{
    class StalkParticleSystem : ParticleSystem
    {
        public StalkParticleSystem()
        {
            m_PST = ParticleSystemType.Stalk;
        }
        ...
        public override void AdvanceSystem(double fTime, double fElapsedTime, in XMFloat3 vRight, in XMFloat3 vUp, in XMFloat3 vWindVel, in XMFloat3 vGravity)
        {
            if (m_fCurrentTime > 0)
            {
                for (int i = m_pParticlesStartIndex; i < m_pParticlesStartIndex + m_NumParticles; i++)
                {
                    ...
                    XMVector vDeltaXZ = vDelta;
                    vDeltaXZ.Y = 0.0f;

                    float fLenSq = XMVector3.LengthSquare(vDeltaXZ).X / m_fSpread;
                    float fExp = Math.Min(vWindVel.Y, (1.0f / fLenSq)); //exp( -fLenSq );
