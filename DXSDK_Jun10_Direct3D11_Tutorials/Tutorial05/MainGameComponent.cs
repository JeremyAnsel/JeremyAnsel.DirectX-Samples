...
namespace Tutorial05
{
    class MainGameComponent : IGameComponent
    {
        ...
        private XMMatrix worldMatrix1;

        private XMMatrix worldMatrix2;
        ...
        public void Render()
        {
            ...
            ConstantBufferData cb1;
            cb1.World = this.worldMatrix1.Transpose();
            cb1.View = this.viewMatrix.Transpose();
            cb1.Projection = this.projectionMatrix.Transpose();
            context.UpdateSubresource(this.constantBuffer, 0, null, cb1, 0, 0);

            // Render the first cube
            context.DrawIndexed((uint)MainGameComponent.Indices.Length, 0, 0);

            // Update variables for the second cube
            ConstantBufferData cb2;
            cb2.World = this.worldMatrix2.Transpose();
            cb2.View = this.viewMatrix.Transpose();
            cb2.Projection = this.projectionMatrix.Transpose();
            context.UpdateSubresource(this.constantBuffer, 0, null, cb2, 0, 0);
