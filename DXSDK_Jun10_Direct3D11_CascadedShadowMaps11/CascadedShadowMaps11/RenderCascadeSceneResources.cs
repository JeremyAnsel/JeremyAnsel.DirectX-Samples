...
{
    static class RenderCascadeSceneResources
    {
...
float4 PSMain( VS_OUTPUT Input ) : SV_TARGET
{
    ...
    float fLighting = 
                      saturate( dot( vLightDir1 , Input.vNormal ) )*0.125f +
                      saturate( dot( vLightDir2 , Input.vNormal ) )*0.125f +
                      saturate( dot( vLightDir3 , Input.vNormal ) )*0.125f +
                      saturate( dot( vLightDir4 , Input.vNormal ) )*0.125f ;
    ...
    return float4((fLighting * vVisualizeCascadeColor * vDiffuse).xyz, 1.0f);
}
";
    }
}
